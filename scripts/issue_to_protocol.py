...
from common import ROOT, PROTOCOLS_DIR, load_tools, load_vocab, reverse_label_maps, slugify
from issue_to_tool import parse_sections, clean, selected, map_values, parse_contact

LABELS={
 'name':'Protocol or method name',
 'publication':'Publication DOI or URL',
 'summary':'What problem does this protocol solve?',
...
def main():
    ...
    name=value('name')
    if not name: raise ValueError('Protocol name is missing')
    slug=slugify(name); path=PROTOCOLS_DIR/f'{slug}.yml'
